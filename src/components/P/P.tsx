...
const P = (props: PProps) => {
  ...
  if (typeof text === 'string') {
    if (text.includes('{% hint')) {
      ...
      const textMatches = text.match(/{%\s*hint\s*style="\w+"\s*%}\s*(.*)/);

      const style =
        styleMatches && styleMatches[1]
      ...
      return <VideoPlayer url={linkComponent.props.href} caption={caption} />;
      ...
      return <SideNote text={text}>{children}</SideNote>;
